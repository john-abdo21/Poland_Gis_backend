...
from .models import PlRiver3857, PlPlot3857,Inlandwater
...
def complex_Search(request):
    if request.method == 'GET':
        ...
        '''
        sqlForLake = '''
            SELECT ST_AsGeoJSON(ST_Union(ST_Buffer(ST_Transform(shape, 3857), %(buffer_distance)s))) AS merged_geojson FROM inlandwater WHERE area >= %(r_Area)s;
        '''
        ...
        with connection.cursor() as cursor:
            cursor.execute(sqlForLake, {'buffer_distance': l_distance, 'r_Area': l_area})
...
def all_Search(request):
    if request.method == 'GET':
        ...
        if r_length:
            ...
            if f_area_max:
                sqlForForest = '''
                    SELECT ST_AsGeoJSON(ST_Union(ST_Transform(way, 3857))) AS merged_geojson FROM planet_osm_polygon WHERE way_area >= %(f_Area_min)s AND way_area >= %(f_Area_max)s AND landuse='forest';
                '''
                with connection.cursor() as cursor:
                ...
                sqlForForest = '''
                        SELECT ST_AsGeoJSON(ST_Union(ST_Transform(way, 3857))) AS merged_geojson FROM planet_osm_polygon WHERE way_area >= %(f_Area_min)s AND landuse='forest';
                    '''
                with connection.cursor() as cursor:
